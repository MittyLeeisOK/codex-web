...
| Startup phase smoothing | The watchdog now advances phases on a short elapsed-time cadence while still using real Codex DOM readiness to exit. Desktop validation showed `2/6`, `3/6`, and `4/6` before the app UI took over, avoiding the previous `2/6`-then-complete jump without intentionally delaying fast starts. |
